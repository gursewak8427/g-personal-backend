...
io.on("connection", (socket) => {
  ...
  socket.on("upload_csv_websocket_custom_validation", async (data) => {
    try {
      let countSchools = await SchoolModel.find();
      countSchools = countSchools.length;
      ...
      csv()
        ...
        .then(async (jsonObj) => {
          ...
          for (var index = 0; index < jsonObj.length; index++) {
            ...
            if (
              jsonObj[index]["school_name"] == "" &&
              jsonObj[index]["program_name"] == ""
            )
              continue;
            ...
            if (addedSchools.includes(jsonObj[index]["school_name"])) {
              ...
              var new_stream = jsonObj[index]["new_stream"]
                .split(",")
                .map((item) => Constants.new_stream[parseInt(item)]);
              var program_level = jsonObj[index]["program_level"]
                .split(",")
                .map((item) => Constants.program_level[parseInt(item)]);
              ...
              let intakeNewData = jsonObj[index]["intake_id"]
                .split(",")
                .map((intake) => {
              ...
              finalObj[myindex]["school_programs"].push({
                program_id: `P${countSchools}${index + 1}`,
                program_name: jsonObj[index]["program_name"],
                description: jsonObj[index]["description"],
                duration: parseFloat(jsonObj[index]["duration"]) || null,
                duration_sem_per_year:
                  parseFloat(jsonObj[index]["duration_sem_per_year"]) || null,
                grade_score: parseFloat(jsonObj[index]["grade_score"]) || 0,
                overall_band: parseFloat(jsonObj[index]["overall_band"]) || 0,
                pte_score: parseFloat(jsonObj[index]["pte_score"]) || 0,
                tofel_point: parseFloat(jsonObj[index]["tofel_point"]) || null,
                ...
                min_tution_fee_per_semester:
                  parseFloat(
                    jsonObj[index]["min_tution_fee_per_semester"]
                      .split(",")
                      .join("")
                  ) || 0,
                max_tution_fee:
                  parseFloat(
                    jsonObj[index]["max_tution_fee"].split(",").join("")
                  ) || 0,
                cost_of_living:
                ...
                other_comment: jsonObj[index]["other_comment"],
                foundation_fee:
                  parseFloat(jsonObj[index]["foundation_fee"]) || 0,
                acceptable_band:
              ...
              // get school detail
              var schoolDtl = await SchoolNamesModel.aggregate([
                {
                  $match: {
              ...
              finalObj.push({
                ...
                total_student:
                  parseFloat(
                    jsonObj[index]["total_student"].split(",").join("")
                  ) || 0,
                ...
                school_programs: [
                  {
                    program_id: `P${countSchools}1`,
                    program_name: jsonObj[index]["program_name"],
                    description: jsonObj[index]["description"],
                    duration: parseFloat(jsonObj[index]["duration"]) || null,
                    duration_sem_per_year:
                      parseFloat(jsonObj[index]["duration_sem_per_year"]) ||
                      null,
                    grade_score: parseFloat(jsonObj[index]["grade_score"]) || 0,
                    overall_band:
                      parseFloat(jsonObj[index]["overall_band"]) || 0,
                    pte_score: parseFloat(jsonObj[index]["pte_score"]) || 0,
                    tofel_point:
                      parseFloat(jsonObj[index]["tofel_point"]) || null,
                    ielts_listening:
                      parseFloat(jsonObj[index]["ielts_listening"]) || 0,
                    ielts_speaking:
                      parseFloat(jsonObj[index]["ielts_speaking"]) || 0,
                    ielts_writting:
                      parseFloat(jsonObj[index]["ielts_writting"]) || 0,
                    ielts_reading:
                      parseFloat(jsonObj[index]["ielts_reading"]) || 0,
                    new_stream: new_stream,
                    stream_id: parseFloat(jsonObj[index]["stream_id"]) || 0,
                    other_fees: parseFloat(jsonObj[index]["other_fees"]) || 0,
                    application_fee:
                    ...
                    visa_processing_days:
                      parseFloat(jsonObj[index]["visa_processing_days"]) || 0,
                    process_days:
                      parseFloat(jsonObj[index]["process_days"]) || 0,
                    credentials: jsonObj[index]["credentials"] || "",
                    program_level: program_level,
                    other_comment: jsonObj[index]["other_comment"],
                    foundation_fee:
            ...
            if (!newSchool) {
              ...
              for (var j = 0; j < singleSchool.school_programs.length; j++) {
                ...
                if (erroredData[myCustomIndex] != "NOT_ERROR") {
                  socket.emit("FromAPI", {
                    message: "Failed",
                    details: erroredData[myCustomIndex],
                    index: myCustomIndex++,
                  });
                } else {
                  try {
