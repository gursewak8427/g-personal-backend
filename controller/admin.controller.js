            ...
            try {
                ...
                jwt.verify(
                    ...
                    async (err, decodedToken) => {
                        ...
                        const loginToken = jwt.sign(data, jwtSecretKey, { expiresIn: "1d" });
    ...
    try {
        ...
        if (admin) {
            ...
            bcrypt.hash(password, saltRounds, async function (err, hash) {
                ...
                res.json({
                    status: 1,
                    message: role == "SUBADMIN" ? "Sub Admin Register Successfully" : "Counselor Register Successfully",
                    details: {
                        admin: {
                            email: admin.email,
                            id: admin._id,
...
const getStudents = async (req, res) => {
    try {
        ...
        if (data?.agentId) {
            let totalStudents = await StudentModel.find({ agent_id: data.agentId }).sort({ "createdAt": "-1" })
            students = await StudentModel.find({ agent_id: data.agentId }).sort({ "createdAt": "-1" }).skip((perPage * (currentPage - 1)) || 0).limit(perPage)
...
const getAgents = async (req, res) => {
    try {
        ...
        let agents = await AgentModel.find(q1).skip((perPage * (currentPage - 1)) || 0).limit(perPage)
...
const toggleStatus = async (req, res) => {
    try {
        ...
        let agent = await AgentModel.findOne({ _id: agentId != "" ? ObjectId(agentId) : "" });
        if (!agent) {
            res.json({
                status: "0",
            ...
            let msg = `Your Id is ${status == "APPROVED" ? "approved" : "rejected"} as Agent`
...
const getSchools = async (req, res) => {
    try {
        ...
        if (data?.countryId) {
            // query of country
            const cntryDtl = await CountryModel.findOne({ countryId: data.countryId })
        ...
        res.json({
            ...
            details: {
                schools: finalSchools, totalPages, currentPage,
                ...
                $group: {
                    ...
                    school_programs: {
                        $push: {
                            ...
                            min_tution_fee_per_semester: "$school_programs.min_tution_fee_per_semester",
...
const toggleFewSeatsStatus = async (req, res) => {
    ...
    if (myIndex != null) {
        school.school_programs[myIndex].few_seats_status = !school.school_programs[myIndex].few_seats_status
...
const toggletopstatus = async (req, res) => {
    ...
    console.log({ myIndex, sId, pId })
    if (myIndex != null) {
        school.school_programs[myIndex].top_status = !school.school_programs[myIndex].top_status
...
const addCountry = async (req, res) => {
    try {
        ...
        res.json({ message: "country upload successfully", status: "1", details: { newCountry } })
    } catch (error) {
        console.log(error)
...
const updateCountry = async (req, res) => {
    try {
        ...
        res.json({ message: "country Updated successfully", status: "1", details: { updatedCountry: oldCountry } })
    } catch (error) {
        console.log(error)
        res.json({ message: "Server Error", status: "0" })
    }
...
const addSchoolname = async (req, res) => {
    ...
    res.json({ message: "school name upload successfully", status: "1", details: { newSchoolName: newSchoolDetails[0] } })
...
const updateSchoolName = async (req, res) => {
    try {
        const { id, state, city, countryName } = req.body;
        ...
        res.json({ message: "School Detail Updated successfully", status: "1", details: { updatedSchoolName: newSchoolDetails[0] } })
    } catch (error) {
        console.log(error)
        res.json({ message: "Server Error", status: "0" })
    }
...
const adminList = async (req, res) => {
    try {
        ...
        students = await AdminModel.find({ role: { $ne: "ADMIN" } }).skip((perPage * (currentPage - 1)) || 0).limit(perPage)
        totalPages = parseInt(totalStudents.length / perPage)
...
const getAssessmentForms = async (req, res) => {
    try {
        ...
        students = await AssessmentForm.find().sort({ "createdAt": "-1" }).skip((perPage * (currentPage - 1)) || 0).limit(perPage)
        totalPages = parseInt(totalStudents.length / perPage)
...
const getSearchQueryForms = async (req, res) => {
    try {
        ...
        students = await QueriesForm.find().sort({ "createdAt": "-1" }).skip((perPage * (currentPage - 1)) || 0).limit(perPage)
        totalPages = parseInt(totalStudents.length / perPage)
...
const findIntakes = async (req, res) => {
    ...
    for (let schoolIndex = 0; schoolIndex < schoolsList.length; schoolIndex++) {
        ...
        for (let programIndex = 0; programIndex < school.school_programs.length; programIndex++) {
            ...
            for (let intekeIndex = 0; intekeIndex < program.intakes_data.length; intekeIndex++) {
                ...
                if (intakeData) {
                    ...
                    for (let monthIndex = 0; monthIndex < intakeData.months.length; monthIndex++) {
                ...
                let findIndex = -1;
                for (let globalIntakeIndex = 0; globalIntakeIndex < globalIntake.length; globalIntakeIndex++) {
                ...
                if (findIndex == -1) {
                    ...
                    for (let monthIndex = 0; monthIndex < demoIntake.months.length; monthIndex++) {
...
const updateIntakes = async (req, res) => {
    ...
    try {
        ...
        for (let index = 0; index < schoolsList.length; index++) {
            ...
            for (let programIndex = 0; programIndex < findSchool.school_programs.length; programIndex++) {
                if (programId) {
                    if (findSchool.school_programs[programIndex].program_id != programId) {
                        continue;
                    }
                }
